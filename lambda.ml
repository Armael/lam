...
let rec cps e =
  ...
  *)
         ...
         (lam [vv]
            (cont (cps f)
               (lam [fv]
                  (cont (cps stack)
                     (lam [stackv]
                        (app (Var stackv) [Var fv; Var vv; Var k])))))))
  ...
  | Perform e ->
    ...
    let stack =
      lam [f; x; k'] (
        app (Var f) [Var x; Var k; Var hx; Var hf; Var k'])
    in
